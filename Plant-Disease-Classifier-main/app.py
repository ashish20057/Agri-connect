...
def predict(image_file, model, transform, label_encoder, device):
    ...
    try:
        ...
        temp_file.close()  # Close the file handle so other processes/functions can access it
        ...
        if os.path.exists(temp_path):
            try:
                ...
                pass # Fail silently on cleanup if needed
...
def display_prediction(original_class, formatted_class, confidence, top_classes, formatted_top_classes, top_probabilities):
    ...
    styled_df = prediction_df.style.format({"Confidence": "{:.2f}%"}).background_gradient(cmap='Greens', subset=['Confidence'])
    st.table(styled_df)
...
def display_disease_info(class_name, class_names):
    ...
    if class_name in disease_info:
        ...
        st.markdown("<h3 class='disease-info-header'>Disease Information</h3>", unsafe_allow_html=True)
        ...
        with col1:
            st.markdown("<h4 class='disease-info-subheader'>Description</h4>", unsafe_allow_html=True)
            st.write(info["description"])

            st.markdown("<h4 class='disease-info-subheader'>Causes</h4>", unsafe_allow_html=True)
            st.write(info["causes"])

        with col2:
            st.markdown("<h4 class='disease-info-subheader'>Treatment</h4>", unsafe_allow_html=True)
            st.write(info["treatment"])

            st.markdown("<h4 class='disease-info-subheader'>Prevention</h4>", unsafe_allow_html=True)
...
def main():
    ...
    with col1:
        ...
        st.markdown("<h3 class='sub-header'>Or try an example:</h3>", unsafe_allow_html=True)
        ...
        if st.session_state.uploaded_file is not None:
            ...
            if isinstance(st.session_state.uploaded_file, str):
                ...
                with open(st.session_state.uploaded_file, "rb") as f:
                    file_content = f.read()
                    uploaded_file_obj = type('obj', (object,), {'getvalue': lambda: file_content})

                # Make prediction
                st.session_state.prediction_results = predict(uploaded_file_obj, model, transform, label_encoder, device)
                ...
                # Make prediction
                st.session_state.prediction_results = predict(st.session_state.uploaded_file, model, transform, label_encoder, device)
                st.session_state.image_processed = True
        else:
            # Placeholder when no image is uploaded
            st.info("Please upload an image or select an example to see the preview and diagnosis.")
